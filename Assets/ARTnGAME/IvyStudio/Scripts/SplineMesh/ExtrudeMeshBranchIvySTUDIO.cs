...
public class ExtrudeMeshBranchIvySTUDIO: MonoBehaviour {

        //v0.1
        public float circleRadius = 0.1f;
        public int circleSides = 8;
        public float growThicknessFactor = 0.1f;
	...
	private SplineMeshExtrusionIvySTUDIO.Edge[] precomputedEdges ;
	
		public List<ExtrudedBranchSection> sections; 

		//GROW ONCE
		public SplinerPTREANT SplinePath;
		public SplinerPTREANT SplineSection;
	...
	MeshFilter MESH;
		...
		public ExtrudeMeshBranchIvySTUDIO Parent_branch;//keep link to parent structures, grab section in real time
	...
	void LateUpdate () {
		...
		if (sections.Count < 2)
			...
			if(self_growth){
			 if(branch_level < max_growth_level 
				...
				//grab script
				ExtrudeMeshBranchIvySTUDIO Child_branch_grower = Instances.GetComponentInChildren(typeof(ExtrudeMeshBranchIvySTUDIO)) as ExtrudeMeshBranchIvySTUDIO;
		...
		List<Vector2> sourceMeshData = CreateCircle(circleRadius, circleSides);
		...
		for(int i=0;i<sourceMeshData.Count;i++){
			...
			precomputedEdges = SplineMeshExtrusionIvySTUDIO.BuildManifoldEdges(temp);		
				...
				if(is_root){
                    SplineMeshExtrusionIvySTUDIO.ExtrudeMesh (temp, MESH.mesh, finalSections, precomputedEdges, invertFaces, true,insert_cap_to_bark, growThicknessFactor);
				}else{
                    SplineMeshExtrusionIvySTUDIO.ExtrudeMesh (temp, MESH.mesh, finalSections, precomputedEdges, invertFaces, false,insert_cap_to_bark, growThicknessFactor);
				}		
		}
	}//END UPDATE
