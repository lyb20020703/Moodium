...
namespace Artngame.INfiniDy {
	...
	[ExecuteInEditMode]
	public class ControlCombineChildrenINfiniDyGrassLAND : MonoBehaviour {

        //v1.7 - Above 64K meshes enable - v1.9.9.7
        public bool enableAbove64KMesh = true;
								...
								if (filters != null | 1 == 0) {
									if (filters.Length > 0 | 1 == 0) {
										...
										for (int i=Group_start; i<Group_end; i++) {									
                                            ...
                                            //v1.7
                                            int maxVerts = 32000;
                                            if (enableAbove64KMesh)
                                            {
                                                maxVerts = 3200000;
                                            }

                                            if (!Multithreaded | (Multithreaded & (
										                                      (filter.sharedMesh != null && filter.sharedMesh.vertexCount <= maxVerts && 
                                                ...
                                                MeshCombineUtilityINfiniDyGrassLAND.MeshInstance instance = new MeshCombineUtilityINfiniDyGrassLAND.MeshInstance ();
										...
										foreach (DictionaryEntry de  in materialToMesh) {
                                            ...
                                            MeshCombineUtilityINfiniDyGrassLAND.MeshInstance[] instances = (MeshCombineUtilityINfiniDyGrassLAND.MeshInstance[])elements.ToArray (typeof(MeshCombineUtilityINfiniDyGrassLAND.MeshInstance));
											...
											for (int j=0; j<Split_index.Count; j++) {	
												...
												if (!Multithreaded) {
                                                    ...
                                                    MeshCombineUtilityINfiniDyGrassLAND.MeshInstance[] instances_Split = new MeshCombineUtilityINfiniDyGrassLAND.MeshInstance[Group_end - Group_start];
													...
													//v1.2
													if (Application.isPlaying) {
                                                        if (enableAbove64KMesh)//v1.7
                                                        {
                                                            filter.mesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32; //v1.7
                                                        }
                                                        filter.mesh = MeshCombineUtilityINfiniDyGrassLAND.Combine (instances_Split, generateTriangleStrips);
                                                       
                                                    } else {
                                                        if (enableAbove64KMesh && filter.sharedMesh != null)//v1.7
                                                        {
                                                            filter.sharedMesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32; //v1.7
                                                        }                                                        
                                                        filter.sharedMesh = MeshCombineUtilityINfiniDyGrassLAND.Combine (instances_Split, generateTriangleStrips);
													...
													if (!all_threads_started) {
														...
														int count = 0;
														foreach (MeshCombineUtilityINfiniDyGrassLAND.MeshInstance combine in instances_Split) {
															if (combine.mesh) {												
                                                        ...
                                                        ControlCombineChildrenINfiniDyGrassLAND.Meshy mesh = new ControlCombineChildrenINfiniDyGrassLAND.Meshy ();
														mesh.thread_ended = false;
														MeshList.Add (mesh);
														MeshIDList.Add (0); //signal this thread is not done yet
													
														int counter_mesh = List_increase;
														LoomINfiniDyPANGAEA.RunAsync (() => {
														
															MeshList [counter_mesh] = MeshCombineUtilityINfiniDyGrassLAND.CombineM (counter_mesh, Has_mesh, all_threads_started, instances_Split, 
								...
								for (int i = 0; i<Material_list.Count; i++) {
									...
									for (int j=0; j<Splits[i]; j++) {
										...
										if (MeshList [mesh_counter].vertices != null & MeshIDList [mesh_counter] != 1) { 
                                            ...
                                            string name = "Combined mesh";									
											if (MeshList [mesh_counter].vertices.Length > maxVerts) {
												name = "Combined mesh64";
											}
											...
											if (MeshList [mesh_counter].vertices.Length > 0) {
												...
												if (Application.isPlaying) {
													...
													if (filter.sharedMesh != null) {
                                                        ...
                                                        filter.sharedMesh.name = MeshList [mesh_counter].name;			
