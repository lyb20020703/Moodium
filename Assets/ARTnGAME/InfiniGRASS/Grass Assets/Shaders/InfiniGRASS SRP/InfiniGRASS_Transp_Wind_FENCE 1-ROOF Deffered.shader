...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF Deffered" {
    Properties {
        ...
        _OceanCenter("Ocean Center", Vector) = (0, 0, 0, 0)
         ...
         _RippleScale ("Vary Height", Float ) = 0     
           ...
           _ColorGlobal ("Global tint", Color) = (0.95,0.95,0.95,0) //0.5,0.8,0.5
           ...
           _BaseLight("Grass Base light control", Float) = 0
            _BaseColorYShift("Shift Base color Y axis", Float) = 1
             ...
             _LightDir ("_LightDir", Float) = (0.5,0.5,0.5,0)
             _LightColor ("LightColor", Color) = (0.9,0.9,0.9,1) //0.5,0.8,0.5
    }
    SubShader {
        Tags {
            "Queue"="AlphaTest"
...
//                #endif
            ...
            VertexOutput vert (VertexInput v) {
				...
				UNITY_SETUP_INSTANCE_ID(v); //v1.7.8 //VR1 //UNITY_VERTEX_OUTPUT_STEREO //VR1
				UNITY_INITIALIZE_OUTPUT(VertexOutput, o); //Insert //VR1  // UNITY_VERTEX_INPUT_INSTANCE_ID //v1.7.8 //VR1 
				UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(o); //Insert //VR1

                o.uv0 = v.texcoord0;
                o.vertexColor = v.vertexColor;
                o.normalDir = UnityObjectToWorldNormal(v.normal);//mul(float4(v.normal,0), _World2Object).xyz;
           ...
           //   v.vertex.xyz += (normalize((float3(1,0.5,0.5)+v.normal))*node_389.r*sin(((node_389.b*3.141592654)+node_392.g+node_392.b))*0.16);                
            ...
            void frag(
                ...
                float facing : VFACE)
              {
                ...
                float3 normalDirection =  normalize(mul((normalLocal), -(i.normalDir))); // Perturbed normals
                
                float nSign = sign( dot( viewDirection, normalLocal) ); // Reverse normal if this is a backface
               // i.normalDir *= nSign;
               normalDirection  *= nSign;
            	...
            	//    clip(node_1.a - _Cutoff);
               	...
               	// half Alpha = tex2D(_Diffuse,i.uv0).a;
               	// clip(Alpha - _Cutoff);
				...
				//                    #endif
                //#else
                    float3 lightDirection = normalize(_LightDir); //normalize(_WorldSpaceLightPos0.xyz);
                //#endif               
                
                //lightDirection =  normalize(reflect(_WorldSpaceLightPos0.xyz,normalDirection));   
                lightDirection =  normalize(reflect(_LightDir,normalDirection));             
               ...
               float dist = distance(_OceanCenter, float3(_WaveControl1.x*i.posWorld.y,_WaveControl1.y*i.posWorld.x,_WaveControl1.z*i.posWorld.z) );
                ...
                float node_331 = 1.0;
                float3 diffuseC = (lerp(float3(node_331,node_331,node_331),float3(0.9632353,0.8224623,0.03541304),i.vertexColor.b)*_Diffuse_var.rgb); // Need this for specular when using metallic
                diffuseC = DiffuseAndSpecularFromMetallic( diffuseC, specularColor, specularColor, specularMonochrome );
				...
				//if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y-1)) * col.r* col.r* col.r* 1+0.01)
				if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y+0)) * col.r+0.001)
	            { 	                  
	                //col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*finalColor*8;  	
	                col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z);                                     
	            } 

	            ///////////NEW DEFFERED
	    //      outDiffuse = half4(col*1.5* _LightColor.xyz*_LightColor.w, 1);
				outDiffuse = half4(col*1.5* _LightColor.xyz*_LightColor.w, 1) * 8;

                //outSpecSmoothness = half4(specular*_SpecularPower*3, gloss/5);
                // outSpecSmoothness = half4(specularColor, gloss/5);
        //        outSpecSmoothness = half4(specular*_SpecularPower*2, gloss/5);
				outSpecSmoothness = half4(specular*_SpecularPower * 4, gloss / 5);

                //outNormal = half4(-normalDirection/viewReflectDirection,1);
                //outNormal = half4(-normalDirection,0);
                //outNormal = half4(specular*_SpecularPower,1);
                // outNormal = half4(specular*_SpecularPower+normalDirection,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w*normalDirection,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w-normalDirection*0.5,1);
                //outNormal = half4(specular*_SpecularPower*_LightColor.w-NdotL*normalDirection*5.5,1);
          //      outNormal = half4(specular*_SpecularPower*_LightColor.w-normalize(dot(viewDirection,lightDirection))*normalDirection*1.5,1);
			//	outNormal = half4(lightDirection, 0) + half4(specular*_SpecularPower + normalDirection, 1);

				float nnSign =  (dot(viewDirection, viewDirection))*2;
				outNormal = half4(-i.normalDir * 11- 22*abs(lightDirection), 0);
				outNormal = half4(outNormal.x*1,1.2, outNormal.z,1);
                //o//tEmission = half4(0,0,0,1);
                //outEmission = half4((finalColor/1+col/165)*_LightColor.w,1);
                //outEmission = half4((finalColor/2+col/165)*_LightColor.w*1,1);
           //     outEmission = half4((finalColor*4+col/165)*_LightColor.w*1,1); //v2.0.6
				outEmission = half4((finalColor *6+ col / 165)*_LightColor.w * 1, 1)*0.5; //v2.0.6
                #ifndef UNITY_HDR_ON
...
//                #endif
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {//fixed4 frag(VertexOutput i) : COLOR {
...
/// Final Color:
	            ...
	            {     	       
	                ...
	                col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*finalColor*8;  
	               // col.rgb = (float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*4*+_TimeControl1.z)*(0.3*i.uv0.y+(1-0.1)*finalColor*(1-i.uv0.y));                            
	            }
	            else
	            {
					//col = col * input.color * input.color.a *_UnityTerrainTreeTintColorSM *1.5;						
...
//				if(i.uv0.y >= 1-(3 * (_SnowCoverage+_TimeControl1.y-1)) * node_1.r* node_1.r)
//	            { 
//	            	
//	            }else{
//	            //clip(-1);
