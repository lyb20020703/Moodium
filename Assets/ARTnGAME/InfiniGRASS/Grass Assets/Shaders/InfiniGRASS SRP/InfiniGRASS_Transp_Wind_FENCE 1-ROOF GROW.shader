...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF GROW" {
    Properties {
        ...
        _OceanCenter("Ocean Center", Vector) = (0, 0, 0, 0)
         ...
         _RippleScale ("Vary Height", Float ) = 0     
		   ...
		   shadowBrightness("shadow Brightness", Float) = 1
			   ...
			   //GROW
	   _growStage("_growStage", Range(-1.0, 1.2)) = 0
	       ...
	       //         	v.vertex.y = v.vertex.y *_RandYScale* abs(cos(((_TimeControl1.w*_Time.y +_TimeControl1.z)*0.2 + 2*dist)*_RippleScale));
	          ...
	          // _OceanCenter.z = 0.0;
	           ...
	           // v.vertex =  o.posWorld;
				...
				//GROW
				v.vertex.xyz = v.vertex.xyz - _Thickness * v.normal * smoothstep(0.0, 0.5, v.texcoord0.y - _growStage);
...
/////// Normals:
                ...
                float4 node_1 = tex2D(_Diffuse,TRANSFORM_TEX(node_582.rg, _Diffuse) + float2(0, -_growStage));
...
/// Final Color:
				...
				//GROW
				clip(1 - saturate(i.uv0.x - _growStage) - 0.05);
...
/////// Normals:
                ...
                //float4 node_1 = tex2D(_Diffuse,TRANSFORM_TEX(node_582.rg, _Diffuse)); //v0.8
				float4 node_1 = tex2D(_Diffuse, TRANSFORM_TEX(node_582.rg, _Diffuse) + float2(0, -_growStage));//v0.8
...
//						}
          ...
          //      	v.vertex.y = v.vertex.y *_RandYScale* abs(cos(((_TimeControl1.w*_Time.y +_TimeControl1.z)*0.2 + 2*dist)*_RippleScale));
            ...
            //    clip(node_1.a - _Cutoff);//v0.8
                

				//float4 node_1 = tex2D(_Diffuse, TRANSFORM_TEX(node_582.rg, _Diffuse) + float2(0, -_growStage));//v0.8
				//GROW v0.8
			//	clip(1 - saturate(i.uv0.x - _growStage) - 0.05);
				clip(node_1.a - saturate(i.uv0.x - _growStage) - _Cutoff);//v0.8
