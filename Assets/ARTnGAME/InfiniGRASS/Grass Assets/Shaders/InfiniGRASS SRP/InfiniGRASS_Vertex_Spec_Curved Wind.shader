        ...
        Pass {
           ...
           // }
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                //v.vertexColor = lerp(v.vertexColor, float4(1,0,0,1),0.5);
           ...
           //     v.vertex.xyz += (normalize((float3(1,0.5,0.5)+v.normal))*node_389.r*sin(((node_389.b*3.141592654)+node_392.g+node_392.b))*0.16);
				...
				if( o.uv0.y > _HeightControlPoints.x){					
				...
				if( o.uv0.y > _HeightControlPoints.y){
				...
				if( o.uv0.y > _HeightControlPoints.z){
				...
				if( o.uv0.y > _HeightControlPoints.w){
...
specular *= ((attenuation) * _LightColor0.xyz);////////////// INFINIGRASS FIX !!!! - fixes flickering shadows - disable all above
              ...
              finalColor = lerp(finalColor, finalColor*_Color,_TintPower*i.uv0.y*(0.9+0.6*cos(i.posWorld.x*0.2)+0.6*sin(i.posWorld.z*0.3)+0.6*sin(i.posWorld.z*0.1+0.1)));
...
////// Lighting:
               ...
               // float attenuation = 0.4;//LIGHT_ATTENUATION(i);
