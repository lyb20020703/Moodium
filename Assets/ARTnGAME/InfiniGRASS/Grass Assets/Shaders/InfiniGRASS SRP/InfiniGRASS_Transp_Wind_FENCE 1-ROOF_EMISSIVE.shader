...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF_EMISSIVE" {
    Properties {
        ...
        _OceanCenter("Ocean Center", Vector) = (0, 0, 0, 0)
         ...
         _RippleScale ("Vary Height", Float ) = 0     
		   ...
		   _EmissionPower("Emission Power", Vector) = (1,1,1,1)
	...
	sampler2D _Emission; uniform float4 _Emission_ST;
	float4	   _EmissionPower;
...
/// Final Color:
				...
				//EMISSION
					...
					float4	EmissionNODE = tex2D(_Emission, i.uv0*_Emission_ST.xy + _Emission_ST.zw);
					col = col + pow(EmissionNODE, _EmissionPower.y) * _EmissionPower.x * _EmissionColor;
