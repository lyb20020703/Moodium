...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF URP1 GROW"
{
	Properties
	{
		...
		[HDR][MainColor] _BaseColor("Color", Color) = (0.5,0.5,0.5,1)
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
	...
	Varyings LitPassVertex(Attributes input)
	{
		...
		//GROW
		input.positionOS.xyz = input.positionOS.xyz - _Thickness * input.normalOS * smoothstep(0.0, 0.5, input.texcoord.y - _growStage);
	...
	//	//END INFINIGRASS 1
		...
		//GROW
		clip(1 - saturate(input.uv.x - _growStage) - 0.05);
		//return color;
		return float4(color.r, color.g, color.b, 1 - saturate(input.uv.x - _growStage) );
		}
	#endif			
			ENDHLSL
		}
	...
	#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Shadows.hlsl"
		...
		clip(1 - saturate(input.uv.x - _growStage) - 0.05);
		//return color;
		return float4(0,0,0, 1 - saturate(input.uv.x - _growStage));
