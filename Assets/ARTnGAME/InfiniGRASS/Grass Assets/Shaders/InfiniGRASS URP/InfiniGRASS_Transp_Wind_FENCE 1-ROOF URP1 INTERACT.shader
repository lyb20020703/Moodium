...
Shader "InfiniGRASS/InfiniGrass Directional Wind ROOF URP1 Interact"
{
	Properties
	{
	...
	[HideInInspector] _Color("Base Color", Color) = (0.5, 0.5, 0.5, 1)
		...
		_SpecularPowerOffset("Specular Offset", Float) = 0
	...
	_BaseLight("Grass Base light control", Float) = 0
		...
		_SpecularColor("_Specular Color", Color) = (1, 1, 1, 0) //0.5,0.8,0.5 //v0.2

		_InteractMaxoffset("Interaction max offset in all axis", Float) = 0.1
	}
	...
	SubShader
	{
		...
		Tags{ "RenderType" = "AlphaTest" "RenderPipeline" = "UniversalPipeline" "IgnoreProjector" = "True" "Queue" = "AlphaTest" }
		LOD 300
...
#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
	...
	float3 _ColorGlobal;
	float _TintPower;
	float _SpecularPower; float _SpecularPowerOffset;
	...
	float _InteractMaxYoffset; float _InteractMaxoffset;
	float _SnowOffset;
	//END INFINIGRASS 1
	...
	Varyings LitPassVertex(Attributes input)
	{
		...
		if (distance(_InteractPos, positionWS) < _StopMotionThreshold * 1) {
			...
			if (input.texcoord.y > 0.19) {
				positionWS.x = positionWS.x - clamp(_InteractSpeed.x*(1 - distA)*(input.texcoord.y - 0.5), -_InteractMaxoffset, _InteractMaxoffset);
				positionWS.z = positionWS.z - clamp(_InteractSpeed.z*(1 - distA)*(input.texcoord.y - 0.15), -_InteractMaxoffset, _InteractMaxoffset);
				positionWS.y = positionWS.y - clamp(_InteractSpeed.y*(1 - distA)*(input.texcoord.y - 0.15), -_InteractMaxoffset, _InteractMaxoffset);
	...
	//	//END INFINIGRASS 1
		...
		//lightDirection = normalize(reflect(_LightDirection.xyz, inputData.normalWS));
		float3 halfDirection = normalize(viewDir + lightDirection);
		float node_3 = 0.2;
		float3 specularColor = float3(node_3, node_3, node_3);
		float gloss = 0.4;
		float specPow = exp2( gloss * 10.0+1.0);
		float3 specular = 3 * pow(max(0, dot(halfDirection, inputData.normalWS)), specPow) * specularColor;
		//specular = specular * (input.uv.y * 2 - 0.5);
		color.rgb += specular * (_SpecularPower+ _SpecularPowerOffset)*0.01*_SpecularColor;
		//color.rgb += surfaceData.specular.rgb * _SpecularPower + 0.5*surfaceData.albedo;
