...
public class LoomANG : MonoBehaviour
{
	...
	static bool _quitting;
		private static LoomANG _current;
	private int _count;
	
		public static LoomANG Current
	{
		get
		{
			Initialize();
	...
	public static void Initialize()
	{
		...
		if (go)
		{
				foreach (var LoomINfiniDy in Resources.FindObjectsOfTypeAll(typeof(LoomANG)).Cast<LoomANG>())
					DestroyImmediate(LoomINfiniDy.gameObject);
				var g = new GameObject("LoomINfiniDy");
				_current = g.AddComponent<LoomANG>();
			_initialized = true;
