...
namespace Artngame.CommonTools {
	...
	[CustomEditor(typeof(SplinerPTREANT))] 

	public class SplinerPTREANTEditor : Editor {

		void Awake()
		{
			script = (SplinerPTREANT)target;

			Editor_Initialized=0;
		}

		//v2.1.22
		SerializedProperty useChildren;
		SerializedProperty updateChildren;
		SerializedProperty childrenSplines;
		SerializedProperty childrenAttachIDs;
		SerializedProperty updateMode;
		SerializedProperty is_root;
		SerializedProperty parentSpline;
        SerializedProperty terrainEditors;//v1.1 //v2.1.23

        #region PARAMETERS
        List<BranchSplineNode[]> Keep_Curve;
        //BranchSplineNode[] Curve; //v2.1
        bool replot =true;
		private SplinerPTREANT script;
		...
		public void OnEnable(){
            ...
            //v1.1  //v2.1.23
            terrainEditors = serializedObject.FindProperty("terrainEditors");

            //v2.1.22
            childrenSplines = serializedObject.FindProperty ("childrenSplines");
			useChildren = serializedObject.FindProperty ("useChildren");
			updateChildren = serializedObject.FindProperty ("updateChildren");
			childrenAttachIDs = serializedObject.FindProperty ("childrenAttachIDs");
			updateMode = serializedObject.FindProperty ("updateMode");
			is_root = serializedObject.FindProperty ("is_root");
			parentSpline = serializedObject.FindProperty ("parentSpline");
		...
		public void  OnSceneGUI () {
			...
			if (Keep_Curve == null | replot ){Keep_Curve = new List<BranchSplineNode[]>();}
			Event _Event = Event.current;
			EventType _Type = _Event.type;
            BranchSplineNode[]  SplinePoints = script.SplinePoints.ToArray();
			...
			if(script.control_points_children !=null){
				if(script.control_points_children.Count !=0){
                    ...
                    //v1.1  //v2.1.23
                    if (script.splineChanged)
                    {
                        //script.updateTerrainOnChange();
                        script.splineChanged = false;
                    }
		...
		void Draw_Spheres(int i, BranchSplineNode[]  SplinePoints, int Control_points_Count){
			if (script.PointGizmo_On) {Handles.color = Color.blue;
//#if UNITY_2020_OR_NEWER
                Handles.SphereHandleCap(i, SplinePoints[i].position, Quaternion.identity, script.Handle_scale * 1f, EventType.Repaint);               
//#else
...
//#endif
		...
		void Draw_Curve(int i, BranchSplineNode[]  SplinePoints, int Control_points_Count){
...
#region Curve
				if (Keep_Curve.Count<(i+1)|replot){
					...
					if(script.Curve1 != null & script.Keep_Curve != null){
						...
						script.Curve1 = new BranchSplineNode[1];	
						script.Keep_Curve = new List<BranchSplineNode[]>();
					}
					//				Curve = script.Curve1;
		...
		void ControlMe(BranchSplineNode[]  SplinePoints, int Control_points_Count){
#region Curve Select
...
#region Curve Deselect
		...
		void Control_in(int i, BranchSplineNode[]  SplinePoints){
		...
		public override void  OnInspectorGUI() {
			...
			EditorGUILayout.PropertyField(is_root);
			EditorGUILayout.PropertyField(updateChildren);
			EditorGUILayout.PropertyField(updateMode);
			EditorGUILayout.PropertyField(parentSpline);
            EditorGUILayout.PropertyField(terrainEditors);//v1.1  //v2.1.23
            EditorGUILayout.PropertyField(childrenSplines,true);
			EditorGUILayout.PropertyField(childrenAttachIDs,true);
			...
			{
				script.SplinePoints.Add(new BranchSplineNode(script.gameObject.transform.position,Quaternion.identity));

				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+25,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+50,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+75,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+100,script.gameObject.transform.position.z),Quaternion.identity));

				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+125,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+150,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+175,script.gameObject.transform.position.z),Quaternion.identity));
				script.SplinePoints.Add(new BranchSplineNode(new Vector3(script.gameObject.transform.position.x,script.gameObject.transform.position.y+200,script.gameObject.transform.position.z),Quaternion.identity));
            ...
            //v1.1 - v2.1.23 Terrain spline grow
            if(GUILayout.Button("Additive Terrain Raising"))
            {
                script.setSplineMode(0);
            }
            if (GUILayout.Button("Replacing Terrain Raising"))
            {
                script.setSplineMode(1);
            }
            if (GUILayout.Button("Renewed Terrain Raising"))
            {
                script.setSplineMode(2);
                //save of state texture
                //script.saveStateTexture();
            }
            if (GUILayout.Button("Renewed Terrain Replacing"))
            {
                script.setSplineMode(3);
                //save of state texture
                //script.saveStateTexture();
            }

            //v2.1.23
            if (GUILayout.Button ("Grow Vines")) {
			...
			if (GUILayout.Button ("Add another point")) {
				...
				script.SplinePoints.Add(new BranchSplineNode(Moved_vector_to_last_point,Quaternion.identity));
			...
			if(script.SplinePoints.Count > 2 ){
				if (GUILayout.Button ("Add point between")) {
					if(selectedSegment != -1){
						...
						script.SplinePoints.Insert(selectedSegment+1, new BranchSplineNode(Moved_vector_to_last_point,Quaternion.identity));
